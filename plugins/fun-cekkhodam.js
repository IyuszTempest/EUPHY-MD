...
module.exports = {
    command: ['khodam', 'cekkodam'],
    category: 'fun',
    noPrefix: true,
    call: async (conn, m, { usedPrefix, command, text }) => {
        if (!text) return m.reply(`Masukkan nama kamu!\n\nContoh:\n${usedPrefix + command} Natalius`);

        // Simpan data khodam agar tidak berubah untuk nama yang sama dalam satu sesi
        conn.khodam = conn.khodam ? conn.khodam : {};
        
        const namaInput = text.trim().toLowerCase();
        let data;

        if (conn.khodam[namaInput]) {
            data = conn.khodam[namaInput];
        } else {
            data = khodamList[Math.floor(Math.random() * khodamList.length)];
            conn.khodam[namaInput] = data;
        }

        let result = `╭━━〔 🔮 *𝙲𝙴𝙺 𝙺𝙷𝙾𝙳𝙰𝙼* 〕━━┓\n`;
        result += `┃\n`;
        result += `┃ 👤 *Nama:* ${text}\n`;
        result += `┃ ✨ *Khodam:* ${data.nama}\n`;
        result += `┃\n`;
        result += `┣━━〔 📖 *𝙿𝙴𝙽𝙹𝙴𝙻𝙰𝚂𝙰𝙽* 〕━━━━┓\n`;
